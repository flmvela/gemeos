...
export function NavigationOverlay({ navigationItems, onNavigate }: NavigationOverlayProps) {
  ...
  const groupedItems = {
    insights: navigationItems.filter(item => 
      ['dashboard', 'analytics', 'reports'].includes(item.id)
    ),
    content: navigationItems.filter(item => 
      ['domains', 'concepts', 'goals', 'exercises'].includes(item.id)
    ),
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="outline"
          size="sm"
          className="fixed top-4 left-4 z-40 shadow-lg bg-background border-border hover:bg-accent"
        >
