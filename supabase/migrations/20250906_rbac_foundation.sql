-- ============================================================
-- RBAC Foundation Migration - Phase 1
-- Simple, scalable role-based access control system
-- Based on hybrid recommendation: simple structure + strategic extensibility
-- ============================================================
...
CREATE POLICY "resources_platform_admin_all" ON public.resources
  FOR ALL TO authenticated USING (auth_is_platform_admin());
...
CREATE POLICY "role_permissions_platform_admin_all" ON public.role_permissions
  FOR ALL TO authenticated USING (auth_is_platform_admin());
...
CREATE INDEX IF NOT EXISTS idx_resources_key ON public.resources(key);
...
CREATE INDEX IF NOT EXISTS idx_role_permissions_expires ON public.role_permissions(expires_at) WHERE expires_at IS NOT NULL;

-- Composite index for main permission check query
CREATE INDEX IF NOT EXISTS idx_role_permissions_lookup ON public.role_permissions(role_id, resource_id);

-- User tenant lookup (uses existing user_tenants table)
...
-- RLS for the view
ALTER VIEW public.permission_details OWNER TO postgres;
